...
func DjangoCheckPassword(sourcePassword string, password string) bool {
	sl := strings.Split(sourcePassword, "$")[2]
	checkPasswordToken := DjangoEncrypt(password, sl)
	if sourcePassword == checkPasswordToken {
		return true
	} else {
		return false
	}
